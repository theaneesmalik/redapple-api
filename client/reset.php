...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["email"])) {
        ...
        if ($userCheck > 0) {
            ...
            $query2 = "UPDATE `customers` SET `rst_pass_code` = '$code' WHERE `email`= '$email'";
...
If you did not forget your password, you can ignore this email.
        ...
        $query4 = "UPDATE `customers` SET `rst_pass_code` = NULL,`password` = '$pass' WHERE `email`= '$email'";
        ($result4 = mysqli_query($dbCon, $query4)) or
            die("database error:" . mysqli_error($dbCon));
        if ($result4) {
            $arr["res"] = $result4;
            $query5 = "SELECT * FROM `customers` WHERE `email`='$email'";
