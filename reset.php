...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["email"])) {
        $email = $_POST["email"];
        $query = "SELECT * FROM `adminprofile` WHERE `email`='$email'";
        ($result = mysqli_query($dbCon, $query)) or
            die("database error:" . mysqli_error($dbCon));
        $userCheck = mysqli_num_rows($result);
        if ($userCheck > 0) {
            $code = rand(999999, 111111);
            $query2 = "UPDATE `adminprofile` SET `rst_pass_code` = '$code' WHERE `email`= '$email'";
...
If you did not forget your password, you can ignore this email.
        ...
        $query3 = "SELECT * FROM `adminprofile` WHERE `email`='$email'";
        ...
        $query4 = "UPDATE `adminprofile` SET `rst_pass_code` = NULL,`password` = '$pass' WHERE `email`= '$email'";
        ($result4 = mysqli_query($dbCon, $query4)) or
            die("database error:" . mysqli_error($dbCon));
        if ($result4) {
            $arr["res"] = $result4;
            $query5 = "SELECT * FROM `adminprofile` WHERE `email`='$email'";
